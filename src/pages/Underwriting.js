...
import '../styles/Underwriting.css'
...
export default function Underwriting() {
  ...
  const fetchRecords = useCallback(async () => {
    const data = await underwritingService.getAll()
    setRecords(data)
  }, [])
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const payload = Object.fromEntries(COLUMNS.map(c => [c.key, form[c.key] || null]))
      if (editingId) {
        const updated = await underwritingService.update(editingId, payload)
        setRecords(prev =>
          prev.map(r => r.id === editingId ? { ...updated, customers: r.customers } : r)
        )
      } else {
        const created = await underwritingService.create(form.customer_id, payload)
        const customer = customers.find(c => c.id === form.customer_id)
        setRecords(prev => [{ ...created, customers: customer }, ...prev])
      }
      handleClose()
    } catch (err) {
      console.error('Underwriting save:', err)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="uw-page">
      <div className="uw-page-header">
        <div>
          <h1 className="uw-page-title">언더라이팅</h1>
          <p className="uw-page-sub">고객별 병력 심사 결과 및 가입 담보 현황</p>
        </div>
        <button className="add-btn" onClick={openNew}>+ 추가</button>
      </div>

      {records.length === 0 ? (
        <p className="uw-page-empty">언더라이팅 기록이 없어요. + 추가 버튼으로 시작하세요.</p>
      ) : (
        <div className="uw-table-scroll">
          <table className="uw-table">
            <thead>
              <tr>
                <th style={{ minWidth: 110 }}>고객</th>
                {COLUMNS.map(col => (
            ...
            <tbody>
              {records.map(record => (
                <tr key={record.id} className="uw-row" onClick={() => openEdit(record)}>
                  <td>
                    <span className="uw-customer-cell">
                      {record.customers?.emoji} {record.customers?.name}
                    </span>
                  </td>
                  {COLUMNS.map(col => (
